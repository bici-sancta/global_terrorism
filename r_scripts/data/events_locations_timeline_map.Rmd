...
for (iyyy in 1970 : 2015)
{
	...
	if (length(year_centroid$events_per_year[year_centroid$iyy == iyyy]) > 0)
	{
		...
		icolor <- cb_palette[2016 - iyyy]
	
		locations_year <- locations[locations$iyear == iyyy, ]
	
#	map_gt_annual <- get_map(location = c(lon = mean(year_centroid$lon),
#								   lat = mean(year_centroid$lat)),
#					zoom = 2,
#					maptype = "toner-lite",
#					color = "color",
...
#			alpha = 0.75
				...
				annotate("text", x = 0, y = 72, size = 15,
						 ...
						 color = icolor) +
			geom_point(data = year_centroid,
					   aes( x = lon[year_centroid$iyy == iyyy], y = lat[year_centroid$iyy == iyyy]),
					   color = "firebrick1",
					   size = 20, shape = 13)
...
#		geom_path(data = locations_2010,
#  			aes(x = longitude, y = latitude,
#  			color = iyear),
#  			size = 2,
#	 		lineend = "round",
#	 		na.rm = TRUE) +
#		scale_colour_gradientn(colours = rev(rainbow(45)),
 # 			breaks = seq(1970, 2015, by = 1))
		)
		dev.off()
	}
}
